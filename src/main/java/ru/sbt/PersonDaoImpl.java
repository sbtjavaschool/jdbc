...
public class PersonDaoImpl implements PersonDao {
    ...
    public Person findById(long id) {
        return selectOne(execute("select p.id as id, p.name as name, c.id as cityId, c.name as cityName from Persons p, City c" +
                " where p.city_id = c.id and p.id = ?", s -> s.setLong(1, id)));
    }

    public List<Person> findByName(String name) {
        return execute("select p.id as id, p.name as name, c.id as cityId, c.name as cityName from Persons p, City c" +
